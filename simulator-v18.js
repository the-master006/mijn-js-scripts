...
(function() {
    ...
    function simulateDayForChart(dayOfYear, pakketIndex) {
        ...
        // Peak sun hours per maand (NL gemiddelden)
        var peakSunHours = [0.73, 1.35, 2.51, 3.72, 4.46, 4.65, 4.56, 4.0, 2.79, 1.77, 0.91, 0.54];
        ...
        // Dagelijkse zonproductie in kWh gebaseerd op echte peak sun hours
        var dailySolarKwh = systemSizeKw * peakSunHours[month];
        
        // Debug logging
        console.log('simulateDayForChart - dayOfYear:', dayOfYear, 'month:', month, 
            'peakSunHours:', peakSunHours[month], 'dailySolarKwh:', dailySolarKwh.toFixed(2));
        ...
        var solarPattern, seasonFactor, seasonName;
        if (dayOfYear >= 80 && dayOfYear < 172) { solarPattern = solarPatterns.spring; seasonFactor = seasonFactors.spring; seasonName = 'spring'; }
        else if (dayOfYear >= 172 && dayOfYear < 264) { solarPattern = solarPatterns.summer; seasonFactor = seasonFactors.summer; seasonName = 'summer'; }
        else if (dayOfYear >= 264 && dayOfYear < 355) { solarPattern = solarPatterns.autumn; seasonFactor = seasonFactors.autumn; seasonName = 'autumn'; }
        else { solarPattern = solarPatterns.winter; seasonFactor = seasonFactors.winter; seasonName = 'winter'; }
        
        // Normaliseer solar pattern
        var solarSum = 0; 
        for (var s = 0; s < 24; s++) solarSum += solarPattern[s];
        
        console.log('  Season:', seasonName, 'solarSum:', solarSum.toFixed(2), 
            'max production kW:', (dailySolarKwh * (solarPattern[12] || solarPattern[11]) / solarSum).toFixed(2));
        ...
        for (var h = 0; h < 24; h++) {
            // Productie in kW: dagelijkse kWh verdeeld over uren volgens patroon
            var production = dailySolarKwh * (solarPattern[h] / (solarSum || 1));
