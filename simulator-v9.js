// AccuThuis Batterij Simulator v9.0 - Simulator Preview in Rapport
// Host dit bestand op GitHub Pages of een andere CDN
console.log('AccuThuis Simulator v9.0 geladen - Simulator preview in rapport');
...
(function() {
    ...
    function buildReportHTML(d, reportDate, recommendation, plusCum, proCum, summerDay, winterDay, springDay, autumnDay) {
        ...
        css += '.sim-config-title { font-size: 1.1em; font-weight: 600; color: #1e293b; margin-bottom: 15px; }';
        css += '.sim-config-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 15px; }';
        css += '.sim-config-item { background: rgba(255,255,255,0.15); padding: 12px; border-radius: 6px; text-align: center; }';
        css += '.sim-config-label { font-size: 0.8em; color: #1e293b; margin-bottom: 5px; }';
        css += '.sim-config-value { font-size: 1.4em; font-weight: 600; color: #1e293b; }';
        css += '.sim-stats { padding: 15px 20px; }';
        css += '.sim-stats-row { display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px; margin-bottom: 10px; }';
        css += '.sim-stat { background: #f8fafc; padding: 12px; border-radius: 6px; text-align: center; border-left: 4px solid #3b82f6; }';
        ...
        css += '.sim-stat.battery { border-left-color: #8b5cf6; background: #A9D6E5; }';
        css += '.sim-stat-label { font-size: 0.7em; color: #64748b; margin-bottom: 4px; }';
        css += '.sim-stat-value { font-size: 1.1em; font-weight: 700; color: #1e293b; }';
        css += '.sim-total { background: linear-gradient(135deg, #10b981 0%, #059669 100%); padding: 15px; text-align: center; color: white; }';
        css += '.sim-total-label { font-size: 0.85em; opacity: 0.9; margin-bottom: 5px; }';
        css += '.sim-total-value { font-size: 1.5em; font-weight: 700; }';
        ...
        css += '@media (max-width: 700px) { .config-grid, .seasonal-grid, .co2-grid { grid-template-columns: 1fr; } .co2-highlight { grid-column: span 1; } .donut-grid { grid-template-columns: 1fr; } .contact-buttons, .download-buttons { flex-direction: column; } .section { padding: 20px; } .sim-config-grid, .sim-stats-row { grid-template-columns: 1fr 1fr; } }';
        css += '@media print { ';
        ...
        css += '.header-title, .header-subtitle, .report-date { color: white !important; }';
        ...
        css += '.sim-stat.battery { border-left-color: #8b5cf6 !important; background: #A9D6E5 !important; }';
        ...
        var currentResult = d.proPakket; // Gebruik Pro als standaard weergave
        var currentName = 'Pro Pakket (14 kWh)';
        var currentROI = currentResult.roiYears ? currentResult.roiYears + ' jaar' : '-';
        
        html += '<section class="section"><h2 class="section-title">Uw Simulatie Resultaat</h2>';
        html += '<p class="section-intro">Hieronder ziet u het resultaat van uw simulatie. We tonen het Pro Pakket als voorbeeld omdat dit voor de meeste huishoudens de beste balans biedt tussen investering en opbrengst. In de volgende sectie vergelijken we alle pakketten.</p>';
        ...
        html += '<div class="sim-config-item"><div class="sim-config-label">Zonnepanelen Vermogen (Wp):</div><div class="sim-config-value">' + d.inputs.solarWp + ' Wp</div></div>';
        html += '<div class="sim-config-item"><div class="sim-config-label">Jaarlijks Verbruik (kWh):</div><div class="sim-config-value">' + d.inputs.annualConsumption + ' kWh</div></div>';
        html += '<div class="sim-config-item"><div class="sim-config-label">Batterij Capaciteit (kWh):</div><div class="sim-config-value">' + currentName + '</div></div>';
        html += '</div></div>';
