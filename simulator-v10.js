// AccuThuis Batterij Simulator v10.0 - Dynamische batterij selectie in rapport
// Host dit bestand op GitHub Pages of een andere CDN
console.log('AccuThuis Simulator v10.0 geladen - Dynamische batterij selectie + prominente ROI');
...
(function() {
    ...
    function generateReport() {
        ...
        // Haal huidige batterij selectie op
        var sliderValue = parseInt(document.getElementById('batteryCapacity').value);
        var batteryMap = isAdvancedMode ? [0, 5, 10, 14, 29, 43] : [0, 5, 14];
        var currentBatteryCapacity = batteryMap[sliderValue];
        
        // Bepaal het huidige result object
        var currentResult;
        if (currentBatteryCapacity === 0) currentResult = d.noBattery;
        else if (currentBatteryCapacity === 5) currentResult = d.plusPakket;
        else if (currentBatteryCapacity === 14) currentResult = d.proPakket;
        else currentResult = simulateScenario(currentBatteryCapacity);
        
        // Bepaal display naam en ROI
        var displayNames = { 0: 'Geen batterij', 5: '5 kWh (Plus)', 10: '10 kWh (Plus+)', 14: '14 kWh (Pro)', 29: '29 kWh (Pro+)', 43: '43 kWh (Pro++)' };
        var pakketPrices = { 0: 0, 5: 2799, 10: 4099, 14: 4399, 29: 7099, 43: 9399 };
        var currentDisplayName = displayNames[currentBatteryCapacity] || 'Geen batterij';
        var currentPakketCost = pakketPrices[currentBatteryCapacity] || 0;
        var currentROI = (currentResult.annualSavings > 0 && currentPakketCost > 0) ? (currentPakketCost / currentResult.annualSavings).toFixed(1) + ' jaar' : '-';
    ...
    function buildReportHTML(d, reportDate, recommendation, plusCum, proCum, summerDay, winterDay, springDay, autumnDay) {
        ...
        css += '.sim-roi-banner { background: linear-gradient(135deg, #1e3a8a 0%, #3b82f6 100%); padding: 15px 20px; display: flex; justify-content: center; align-items: center; gap: 15px; }';
        css += '.sim-roi-label { color: rgba(255,255,255,0.9); font-size: 1em; }';
        css += '.sim-roi-value { color: white; font-size: 1.8em; font-weight: 700; }';
        ...
        css += '.sim-roi-banner { background: linear-gradient(135deg, #1e3a8a 0%, #3b82f6 100%) !important; -webkit-print-color-adjust: exact !important; }';
        css += '.sim-roi-label, .sim-roi-value { color: white !important; }';
        css += '}';
        ...
        html += '<div class="sim-config-item"><div class="sim-config-label">Batterij Capaciteit:</div><div class="sim-config-value">' + currentDisplayName + '</div></div>';
        html += '</div></div>';
        
        // ROI Banner - prominente terugverdientijd
        html += '<div class="sim-roi-banner">';
        html += '<span class="sim-roi-label">⚡ Terugverdientijd:</span>';
        html += '<span class="sim-roi-value">' + currentROI + '</span>';
        html += '</div>';
        ...
        html += '<div class="sim-stat battery"><div class="sim-stat-label">Batterij gebruik</div><div class="sim-stat-value">' + currentResult.batteryUsage + ' kWh</div></div>';
        html += '</div></div>';
